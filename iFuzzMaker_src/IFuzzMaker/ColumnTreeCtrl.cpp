...
//	IPA Add ->
	...
	m_cxTotal = 0;
	m_xPos = 0;
	ZeroMemory( m_arrColWidths, sizeof(m_arrColWidths) );
	ZeroMemory( m_arrColFormats, sizeof(m_arrColFormats) );
//	IPA Add <-
}
...
void CColumnTreeCtrl::OnTreeCustomDraw(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	DWORD dwStyle = GetTreeCtrl().GetStyle();
...
//	IPA Add ->
			...
			crTextBk = OnGetCellBkColor(hItem, 0) & pNMTVCustomDraw->clrTextBk;
			clrText = OnGetCellTextColor(hItem, 0);
//	IPA Add <-
...
//	IPA Add ->
			...
			cellFont = OnGetCellFont( hItem, 0 );
			if( cellFont )
			{
				LOGFONT lf;
...
//						| DT_NOPREFIX | DT_END_ELLIPSIS | dwFormat);
					cellFont = OnGetCellFont( hItem, i );
					if( cellFont )
					{
						LOGFONT lf;
						pOldFont = m_Tree.GetFont();
...
//	IPA Add ->
COLORREF CColumnTreeCtrl::OnGetCellBkColor( HTREEITEM hItem, int nColumn )
{
	return m_Tree.GetBkColor();
}
COLORREF CColumnTreeCtrl::OnGetCellTextColor( HTREEITEM hItem, int nColumn )
{
	return m_Tree.GetTextColor();
}
CFont* CColumnTreeCtrl::OnGetCellFont( HTREEITEM hItem, int nColumn )
{
	return NULL;
}
//	IPA Add <-
